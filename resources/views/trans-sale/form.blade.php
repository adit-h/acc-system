<x-app-layout :assets="$assets ?? []">
   <div>
      ...
      ?>
      @if(isset($id))
      {!! Form::model($data, ['route' => ['trans.sale.update', $id], 'method' => 'patch']) !!}
      @else
      {!! Form::open(['route' => ['trans.sale.store'], 'method' => 'post']) !!}
      @endif
      <div class="row">
            ...
            <div class="card">
               <div class="card-header d-flex justify-content-between">
                  <div class="header-title">
                     <h4 class="card-title">{{$id !== null ? 'Ubah Transaksi Sale' : 'Transaksi Sale Baru' }}</h4>
                  </div>
                  <div class="card-action">
                        <a href="{{route('trans.sale.index')}}" class="btn btn-sm btn-primary" role="button">Back</a>
                  </div>
               </div>
               <div class="card-body">
