<x-app-layout :assets="$assets ?? []">
   <div>
      ...
      ?>
      @if(isset($id))
      {!! Form::model($data, ['route' => ['adjustment.out.update', $id], 'method' => 'patch']) !!}
      @else
      {!! Form::open(['route' => ['adjustment.out.store'], 'method' => 'post']) !!}
      @endif
      <div class="row">
            ...
            <div class="card">
               <div class="card-header d-flex justify-content-between">
                  <div class="header-title">
                     <h4 class="card-title">{{$id !== null ? 'Ubah Adjustment Out' : 'Adjustment Out Baru' }}</h4>
                  </div>
                  <div class="card-action">
                        <a href="{{route('adjustment.out.index')}}" class="btn btn-sm btn-primary" role="button">Back</a>
                  </div>
               </div>
               <div class="card-body">
                  <div class="new-user-info">
                        ...
                        <hr>
                        <button type="submit" class="btn btn-primary">{{$id !== null ? 'Ubah' : 'Tambah' }} Adjustment</button>
                  </div>
               </div>
            </div>
         </div>
