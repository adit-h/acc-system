<x-app-layout :assets="$assets ?? []">
   <div>
      ...
      ?>
      @if(isset($id))
      {!! Form::model($data, ['route' => ['trans.debt.payment.update', $id], 'method' => 'patch']) !!}
      @else
      {!! Form::open(['route' => ['trans.debt.payment.store'], 'method' => 'post']) !!}
      @endif
      <div class="row">
            ...
            <div class="card">
               <div class="card-header d-flex justify-content-between">
                  <div class="header-title">
                     <h4 class="card-title">{{$id !== null ? 'Ubah Transaksi Bayar Hutang' : 'Transaksi Bayar Hutang Baru' }}</h4>
                  </div>
                  <div class="card-action">
                        <a href="{{route('trans.debt.payment.index')}}" class="btn btn-sm btn-primary" role="button">Back</a>
                  </div>
               </div>
               <div class="card-body">
