<x-app-layout :assets="$assets ?? []">
   <div>
      ...
      ?>
      @if(isset($id))
      {!! Form::model($data, ['route' => ['goods.purchase.update', $id], 'method' => 'patch']) !!}
      @else
      {!! Form::open(['route' => ['goods.purchase.store'], 'method' => 'post']) !!}
      @endif
      <div class="row">
            ...
            <div class="card">
               <div class="card-header d-flex justify-content-between">
                  <div class="header-title">
                     <h4 class="card-title">{{$id !== null ? 'Ubah Pembelian' : 'Pembelian Baru' }}</h4>
                  </div>
                  <div class="card-action">
                        <a href="{{route('goods.purchase.index')}}" class="btn btn-sm btn-primary" role="button">Back</a>
                  </div>
               </div>
               <div class="card-body">
                  <div class="new-user-info">
                        ...
                        <hr>
                        <button type="submit" class="btn btn-primary">{{$id !== null ? 'Ubah' : 'Tambah' }} Pembelian</button>
                  </div>
               </div>
            </div>
         </div>
