...
class TransactionAssetTransferDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', 'trans-asset-transfer.action')
            ->rawColumns(['action']);
    }
    ...
    public function query()
    {
        $model = TransactionIn::query()->with('receiveFrom')->with('storeTo')
            ...
            ->whereIn('maf.category_id', [1, 2, 3, 4, 5])
            ->whereIn('mat.category_id', [1, 2, 3, 4, 5]);
    ...
    {
        return 'TransactionAssetTransfer_' . date('YmdHis');
    }
}
